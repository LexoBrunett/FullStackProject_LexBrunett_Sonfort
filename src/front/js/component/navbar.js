...
export const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // Accede a la propiedad is_admin almacenada en localStorage
    const isAdmin = localStorage.getItem("is_admin");
  },[])

  if (store.is_admin == true) {
    return (
      <div
          ...
          >
            <div>
              {!store.auth ? (
                ...
                <div
                  className="d-flex mt-1 py-2"
                  style={{ border: "1px solid gray", borderRadius: "50px" }}
                  onClick={() => actions.logout()}
                >
                  <div className="mx-3" style={{ paddingTop: "3px" }}>
                    Hola, {store.name}
                  </div>
                  <img
                    src={logout}
          ...
          <ul className="nav">
            <li className="nav-item">
              <div className="dropdown">
                <button
                  className="btn btn-success d-flex justify-content-around align-items-center"
                ...
                <ul className="dropdown-menu" style={{ padding: "8px" }}>
                  {store.categories.map((item) => (
                    <li key={item.id} style={{ padding: "2px 12px" }}>
                      <Link
                        to={`/lista_por_categorias/${item.id}`}
                        style={{ textDecoration: "none", color: "#5c6c75" }}
                      >
                        <b>{item.name}</b>
                      </Link>
                    </li>
            ...
            <li>
              <Link to="/product2" style={{ textDecoration: "none" }}>
                <b className="nav-link">Productos</b>
              </Link>
            </li>
            ...
            {store.auth ? (
              <li>
                <Link to="/ordenes" style={{ textDecoration: "none" }}>
                  <b className="nav-link">Mis Ordenes</b>
                </Link>
              </li>
            ) : null}

            {store.auth && store.user.is_admin ? (
              <li>
                <Link to="/Home_admin" style={{ textDecoration: "none" }}>
                  <b className="nav-link">Administrador</b>
                </Link>
              </li>
            ) : null}
            ...
            <li>
              <Link to="/" style={{ textDecoration: "none" }}>
                <b className="nav-link">Contactenos</b>
              </Link>
            </li>
          </ul>
        </header>
